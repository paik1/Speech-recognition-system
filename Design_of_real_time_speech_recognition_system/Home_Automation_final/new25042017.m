...
%  fopen(ser_obj);
Time=6; % the audio recording time
...
% % % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%SVM_Classifier%%%%%%%%%%%
%  PHONEME_NUM_ARRAY=[];
% load HOMO_AUTO_PHONOME_TRAINED_matrix.mat
% for i=1:z-2
...
%      end 
% minX = min(powerarray1);
% maxX = max(powerarray1);
% len = length(powerarray1);
% 
% for j = 1:10
%     idxMin = j;
%     for i = j+1:length(powerarray1)       
%         if(powerarray1(i) > powerarray1(idxMin))
%             idxMin = i;
%         end
%     end
%     tmp1 = powerarray1(idxMin);
%      tmp2 = framesarray1(idxMin);
%       tmp3 = filterbankarray1(idxMin);
%     powerarray1(idxMin) = powerarray1(j);
%     framesarray1(idxMin) = framesarray1(j);
%     filterbankarray1(idxMin) = filterbankarray1(j);
%     newpowerarray1(j) = tmp1;
%     newframesarray1(j) = tmp2;
%     newfilterbankarray1(j) = tmp3;
% end
% %%%%%%%%Sorting based on the frames
% NewFRAME1=[];
% NewFB1=[];
% NewPOWER1=[];
% 
% minX = min(newframesarray1);
% maxX = max(newframesarray1);
% len = length(newframesarray1);
% 
% for j = 1:10
%     idxMin = j;
%     for i = j+1:length(newframesarray1)       
%         if(newframesarray1(i) < newframesarray1(idxMin))
%             idxMin = i;
%         end
%     end
%     tmp4 = newframesarray1(idxMin);
%     newframesarray1(idxMin) = newframesarray1(j);
%     NewFRAME1(j) = tmp4;
%     tmp5 = newfilterbankarray1(idxMin);
%     newfilterbankarray1(idxMin) = newfilterbankarray1(j);
%     NewFB1(j) = tmp5;
%     tmp6 = newpowerarray1(idxMin);
%     newpowerarray1(idxMin) = newpowerarray1(j);
%     NewPOWER1(j) = tmp6;
%     
% end
%delta calculation
