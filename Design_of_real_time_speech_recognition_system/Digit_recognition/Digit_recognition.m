...
clc ;
clear all;
Time=2; % the audio recording time
...
if(LENGTH_PHONEME_ARRAY>=45)
    PHONEME_NUM_ARRAY=[PHONEME_NUM_ARRAY(1:45,:)];
...
for i=1
    ...
    if (out==1)
        ...
        if (out==1)
            if(NCOUNT>=FCOUNT)
                disp('NINE'); 
        elseif(IVECOUNT>=TOCOUNT||FACOUNT>=TOCOUNT)
                 disp('FIVE');
            else
                 disp('FOUR');
             end
        ...
        if (out==1)
            if(NCOUNT>=FCOUNT)
                disp('NINE');
            elseif(IVECOUNT>=TOCOUNT||FACOUNT>=TOCOUNT||NCOUNT<=FCOUNT)
                   disp('FIVE');
        ...
        if (out==1)
            %if(SIHCOUNT>KCOUNT||SCOUNT>KCOUNT)
        disp('SIX');
            %end
        else
