...
% 
%  figure(7)
% plot(newfilterbankarraydkk);
% for i=1:100
%     w(i,1)=floor((i-1)/10);
% end
% 
% svm_struct = svmtrain(refmat,y);
% out = svmclassify(svm_struct,newfilterbankarraydkk);
count=0;
load refmat.mat;
...
%out = svmclassify(svm_struct(1),x);
end
newfilterbankarraydkk=[newfilterbankarraydkk zeros(1,50-length(newfilterbankarraydkk))];
x=newfilterbankarraydkk;
for i=1
    for j=1:9
